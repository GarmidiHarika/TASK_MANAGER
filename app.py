...
def get_db_connection():
    return mysql.connector.connect(**DB_CONFIG) #connection establish between MYsql and flask 
...
@app.route('/')
def index():
    return render_template('index.html') # make a connection between frontend and flask 
...
def get_tasks():
    ...
    if filter_type == 'completed':
        cursor.execute("SELECT * FROM tasks WHERE completed = TRUE ORDER BY id ASC")
    elif filter_type == 'pending':
        cursor.execute("SELECT * FROM tasks WHERE completed = FALSE ORDER BY id ASC")
    else:
